...
describe('ProviderStatusBadge', () => {
  ...
  const mockProvider: SubtitleProvider = {
    id: '1',
    name: 'OpenSubtitles',
    enabled: true,
    type: 'opensubtitles',
  ...
  it('renders disabled status for disabled provider', () => {
    render(
      <ProviderStatusBadge
        provider={{ ...mockProvider, enabled: false, status: 'disabled' }}
        status="disabled"
      />,
    );
  ...
  it('renders error status for provider with error', () => {
    render(
      <ProviderStatusBadge
        provider={{ ...mockProvider, status: 'error' }}
        status="error"
      />,
    );
  ...
  it('displays last error message when status is error', () => {
    ...
    render(
      <ProviderStatusBadge
        provider={{ ...mockProvider, lastError: errorMessage, status: 'error' }}
        status="error"
      />,
    );
  ...
  it('does not display error when status is not error', () => {
    render(
      <ProviderStatusBadge
        provider={{ ...mockProvider, lastError: 'Some error', status: 'active' }}
        status="active"
      />,
    );
  ...
  it('shows correct status based on provider enabled state', () => {
    const { rerender } = render(
      <ProviderStatusBadge provider={mockProvider} status="active" />,
    );
