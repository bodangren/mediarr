...
describe('useMediaQuery', () => {
  ...
  it('updates when media query changes', async () => {
    ...
    act(() => {
      listeners[0]!({ matches: true, media: '(max-width: 768px)' } as MediaQueryListEvent);
    });
  ...
  it('handles multiple media query instances independently', () => {
    const listeners1: Array<(event: MediaQueryListEvent) => void> = [];
    const listeners2: Array<(event: MediaQueryListEvent) => void> = [];
    ...
    const mockMatchMedia1 = {
      ...
      removeListener: vi.fn(),
      addEventListener: vi.fn((event: string, listener: any) => {
        if (event === 'change') listeners1.push(listener);
      }),
      removeEventListener: vi.fn(),
      dispatchEvent: vi.fn(),
    } as unknown as MediaQueryList;
    ...
    const mockMatchMedia2 = {
      ...
      removeListener: vi.fn(),
      addEventListener: vi.fn((event: string, listener: any) => {
        if (event === 'change') listeners2.push(listener);
      }),
      removeEventListener: vi.fn(),
      dispatchEvent: vi.fn(),
    } as unknown as MediaQueryList;
    ...
    act(() => {
      listeners1[0]!({ matches: true, media: '(max-width: 768px)' } as MediaQueryListEvent);
    });
    ...
    act(() => {
      listeners2[0]!({ matches: false, media: '(min-width: 1024px)' } as MediaQueryListEvent);
    });
