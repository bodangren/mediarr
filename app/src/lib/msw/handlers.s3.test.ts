...
async function findHandler(
  ...
  // MSW stores handler paths as relative strings (e.g. "/api/system/status"). To match
 ...
 * Rejects looser matches like `/api/activity/:id` for `/api/activity/export`
 * (the literal `export` would be subsumed by `:id`, but Phase S3 requires a *dedicated*
 * handler at `/api/activity/export`).
 */
