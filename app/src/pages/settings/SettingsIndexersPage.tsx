...
import { IndexerHealthBadge } from '@/components/indexers/IndexerHealthBadge';
...
import type { IndexerHealthSnapshot } from '@/components/indexers/IndexerHealthBadge';
...
export function SettingsIndexersPage() {
  ...
  const [healthSnapshots, setHealthSnapshots] = useState<Map<number, IndexerHealthSnapshot | null>>(new Map());
  ...
  useEffect(() => {
    ...
    void (async () => {
      const snapshotMap = new Map<number, IndexerHealthSnapshot | null>();
      for (const indexer of indexers) {
        try {
          const health = await api.indexerHealthApi.getHealth(indexer.id);
          snapshotMap.set(indexer.id, health.snapshot);
        } catch {
          snapshotMap.set(indexer.id, null);
        }
      }
      setHealthSnapshots(snapshotMap);
    })();
  }, [indexers]);
  ...
  const handleReenable = useCallback(async (indexerId: number) => {
    try {
      ...
      pushToast({
        ...
        message: err instanceof Error ? err.message : 'Failed to re-enable indexer',
        variant: 'error',
      });
    }
  }, []);
            ...
            <li key={indexer.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border-subtle bg-surface-1 p-4 shadow-sm">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <IndexerHealthBadge
                    snapshot={healthSnapshots.get(indexer.id) ?? null}
