...
function renderWithShell(component, pathname = '/') {
    const queryClient = createTestQueryClient();
    return render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(ToastProvider, { children: _jsx(AppShell, { pathname: pathname, children: component }) }) }));
}
beforeEach(() => {
    vi.clearAllMocks();
    // Indexer API mocks
    ...
    updateIndexerMock.mockImplementation((id, input) => {
...
describe('E2E Journey: Indexer Management Flow', () => {
    it('loads indexers page and performs row-level delete action', async () => {
        ...
        renderWithShell(_jsx(IndexersPage, {}), '/indexers');
    ...
    it('loads indexers page and performs bulk delete action', async () => {
        ...
        fireEvent.click(within(alphaRow).getByRole('checkbox', { name: 'Select row' }));
        fireEvent.click(within(betaRow).getByRole('checkbox', { name: 'Select row' }));
        // Perform bulk delete
...
describe('E2E Journey: Search and Grab Flow', () => {
    it('executes search and performs grab action with API verification', async () => {
        renderWithShell(_jsx(SearchPage, {}), '/search');
        ...
        await waitFor(() => {
            expect(searchCandidatesMock).toHaveBeenCalledWith(expect.objectContaining({
                query: 'Dune Part Two',
            }));
        });
        ...
        await waitFor(() => {
            expect(grabReleaseMock).toHaveBeenCalledWith(expect.objectContaining({
                title: 'Dune Part Two 1080p WEB-DL',
                indexer: 'Indexer A',
                magnetUrl: 'magnet:?xt=urn:btih:abc123',
            }));
...
describe('E2E Journey: Settings Configuration Flow', () => {
    it('updates general settings via save button with payload validation', async () => {
        renderWithShell(_jsx(GeneralSettingsPage, {}), '/settings/general');
...
describe('E2E Journey: Mobile Responsiveness Flow', () => {
    ...
    it('accesses mobile bottom navigation and opens More overflow menu', async () => {
        renderWithShell(_jsx("div", { children: "Test Content" }), '/');
...
//# sourceMappingURL=e2e-journeys.test.js.map