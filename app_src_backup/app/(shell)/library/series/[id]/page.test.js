...
function renderPage() {
    ...
    return render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(ToastProvider, { children: _jsx(SeriesDetailPage, {}) }) }));
}
...
describe('series detail page', () => {
    it('renders season accordion, episode grid, and per-episode file status', async () => {
        ...
        expect(within(episodeOneRow).getByText('completed')).toBeInTheDocument();
        expect(within(episodeTwoRow).getByText('wanted')).toBeInTheDocument();
    });
    it('supports episode monitored toggles', async () => {
        renderPage();
        const episodeTwoRow = screen.getByText(/E2: Preparing to Live/).closest('div.grid');
        const monitoredCheckbox = within(episodeTwoRow).getByRole('checkbox');
