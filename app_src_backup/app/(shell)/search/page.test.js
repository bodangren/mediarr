...
function renderPage(queryClient) {
    return render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(SearchPage, {}) }));
}
...
beforeEach(() => {
    ...
    searchCandidatesMock.mockResolvedValue(paginated([
        {
            indexer: 'Indexer A',
            indexerId: 10,
            guid: 'guid-abc123',
...
describe('search page', () => {
    ...
    it('filters results by indexer, category, and quality', async () => {
        searchCandidatesMock.mockResolvedValueOnce(paginated([
            {
                indexer: 'Indexer A',
                indexerId: 10,
                guid: 'guid-a',
    ...
    it('supports override modal fields and sends overridden values to grab request', async () => {
        ...
        await waitFor(() => {
            expect(grabCandidateMock).toHaveBeenCalledWith(expect.objectContaining({
