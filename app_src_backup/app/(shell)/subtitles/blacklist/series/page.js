...
export default function BlacklistSeriesPage() {
    ...
    const blacklistQuery = useApiQuery({
        queryKey: queryKeys.subtitleBlacklistSeries(queryParams),
        queryFn: () => api.subtitleBlacklistApi.listBlacklistSeries(queryParams),
        staleTimeKind: 'list',
        isEmpty: data => data.items.length === 0,
    });
    const clearBlacklistMutation = useMutation({
        mutationFn: () => api.subtitleBlacklistApi.clearBlacklistSeries(),
        onSuccess: () => {
            setShowClearModal(false);
            queryClient.invalidateQueries({ queryKey: ['subtitle-blacklist', 'series'] });
        },
    });
