...
export default function MoviesHistoryPage() {
    ...
    return (_jsxs("section", { className: "space-y-4", children: [_jsxs("header", { className: "flex items-center justify-between gap-4", children: [_jsxs("div", { className: "space-y-1", children: [_jsx("h1", { className: "text-2xl font-semibold", children: "Movies History" }), _jsx("p", { className: "text-sm text-text-secondary", children: "View subtitle download history for movies." })] }), _jsx(Button, { variant: "danger", onClick: () => setShowClearModal(true), children: "Clear History" })] }), _jsx(HistoryFilters, { filters: filters, onChange: setFilters, providers: toMutable(SUBTITLE_PROVIDERS_VALUES), languages: toMutable(SUBTITLE_LANGUAGES), actions: toMutable(SUBTITLE_ACTIONS_VALUES) }), _jsx(QueryPanel, { isLoading: historyQuery.isPending, isError: historyQuery.isError, isEmpty: historyQuery.isResolvedEmpty, errorMessage: historyQuery.error?.message, onRetry: () => void historyQuery.refetch(), emptyTitle: "No history found", emptyBody: "Start downloading subtitles for movies to see history here.", children: _jsx(DataTable, { data: historyQuery.data?.items ?? [], columns: columns, getRowId: row => row.id, pagination: data
                        ? {
                            page,
                        ...
                        : undefined }) }), _jsx(ConfirmModal, { isOpen: showClearModal, title: "Clear Movies History", description: "This will permanently delete all movie subtitle history. This action cannot be undone.", onCancel: () => setShowClearModal(false), onConfirm: () => clearHistoryMutation.mutate(), cancelLabel: "Cancel", confirmLabel: "Clear History", confirmVariant: "danger", isConfirming: clearHistoryMutation.isPending })] }));
}
