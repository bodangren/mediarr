...
import type { WantedSeriesEntry, WantedQueryParams } from '@/lib/api/subtitleWantedApi';
...
export default function WantedSeriesPage() {
  ...
  const queryInput: WantedQueryParams = useMemo(() => {
    const params: WantedQueryParams = { page, pageSize: 25 };
    if (languageFilter) params.languageCode = languageFilter;
    return params;
  }, [page, languageFilter]);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!search.trim()) return wantedQuery.data.items;

    const searchTerm = search.toLowerCase();
    return wantedQuery.data.items.filter(
      item =>
        item.seriesTitle.toLowerCase().includes(searchTerm) ||
  ...
  const searchItemMutation = useMutation({
    mutationFn: ({ seriesId, languageCode }: { seriesId: number; languageCode: string }) =>
      api.subtitleWantedApi.searchSeriesItem(seriesId, languageCode),
    onSuccess: () => {
      pushToast({
        title: 'Search started',
  ...
  const columns: DataTableColumn<WantedSeriesEntry>[] = [
    {
      key: 'seriesTitle',
      header: 'Series',
      render: row => (
        <Link
          href={`/library/series/${row.seriesId}`}
    ...
    },
    {
      ...
      render: row => {
        ...
        return <span className="text-sm text-text-secondary">S{season}E{episode}</span>;
      },
    },
    {
      key: 'episodeTitle',
      ...
      render: row => (
        <span className="text-sm text-text-secondary">{row.episodeTitle}</span>
      ),
    },
    {
      ...
      render: row => (
        <div className="flex flex-wrap gap-1">
          {row.missingLanguages.map(lang => (
            <LanguageBadge
              key={lang}
              languageCode={lang}
              variant="missing"
              onClick={() => searchItemMutation.mutate({ seriesId: row.seriesId, languageCode: lang })}
            />
          ))}
        </div>
      ),
    ...
    <section className="space-y-4">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-semibold text-text-primary">Wanted Episodes</h1>
            <WantedCountBadge />
          </div>
          <p className="text-sm text-text-secondary">Episodes with missing subtitle tracks</p>
        </div>
        <Button
          variant="primary"
          onClick={() => searchAllMutation.mutate()}
          disabled={searchAllMutation.isPending || countQuery.data?.seriesCount === 0}
          className="gap-2"
        >
      ...
      {searchAllMutation.isPending && (
        <SearchProgressIndicator
          isSearching={searchAllMutation.isPending}
          progress={{
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        <label className="flex-1 space-y-1 text-sm">
          <span className="text-text-secondary">Search by series or episode</span>
          <input
            value={search}
            onChange={event => {
              setPage(1);
            ...
            className="w-full rounded-sm border border-border-subtle bg-surface-1 px-3 py-2 text-sm text-text-primary focus:border-accent-primary focus:outline-none focus:ring-1 focus:ring-accent-primary"
            placeholder="Search series or episode title..."
          />
        </label>
        <label className="sm:w-48 space-y-1 text-sm">
          <span className="text-text-secondary">Filter by language</span>
          <input
      ...
      <QueryPanel
        ...
        isEmpty={wantedQuery.isResolvedEmpty || filteredItems.length === 0}
        errorMessage={wantedQuery.error?.message}
        onRetry={() => void wantedQuery.refetch()}
        emptyTitle="No missing subtitles"
        emptyBody={
          search || languageFilter
            ? 'No missing subtitles match your filters'
            : 'All episodes have their required subtitles!'
        }
      >
        <DataTable
          data={filteredItems}
          columns={columns}
          getRowId={row => `${row.seriesId}-${row.episodeId}`}
          pagination={
            data
              ? {
                  page,
                  ...
                  onNext: () =>
                    setPage(p => Math.min(Math.ceil(data.meta.totalCount / queryInput.pageSize!), p + 1)),
                }
              : undefined
          }
        />
