...
describe('QueueRemoveModal', () => {
    ...
    it('renders modal with title and item name', () => {
        render(_jsx(QueueRemoveModal, { ...defaultProps }));
    ...
    it('calls onClose when Cancel button is clicked', async () => {
        const user = userEvent.setup();
        const onClose = vi.fn();
        render(_jsx(QueueRemoveModal, { ...defaultProps, onClose: onClose }));
    ...
    it('calls onConfirm with options when Remove button is clicked', async () => {
        ...
        render(_jsx(QueueRemoveModal, { ...defaultProps, onConfirm: onConfirm }));
        // Check all options
        const checkboxes = screen.getAllByRole('checkbox');
        await user.click(checkboxes[0]);
    ...
    it('resets options when modal is closed and reopened', async () => {
        const user = userEvent.setup();
        const { rerender } = render(_jsx(QueueRemoveModal, { ...defaultProps }));
        // Check some options
        const checkboxes = screen.getAllByRole('checkbox');
        await user.click(checkboxes[0]);
        await user.click(checkboxes[2]);
        ...
        // Close and reopen
        rerender(_jsx(QueueRemoveModal, { ...defaultProps, isOpen: false }));
        rerender(_jsx(QueueRemoveModal, { ...defaultProps, isOpen: true }));
    ...
    it('shows warning for delete files option', () => {
        render(_jsx(QueueRemoveModal, { ...defaultProps }));
        expect(screen.getByText(/Warning: This will delete downloaded files/i)).toBeInTheDocument();
    });
    it('disables buttons when confirming', () => {
        render(_jsx(QueueRemoveModal, { ...defaultProps, isConfirming: true }));
...
//# sourceMappingURL=QueueRemoveModal.test.js.map