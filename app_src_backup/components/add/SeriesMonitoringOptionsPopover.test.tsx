...
import {
  ...
  MONITORING_OPTIONS,
} from './SeriesMonitoringOptionsPopover';

describe('SeriesMonitoringOptionsPopover', () => {
  it('renders with default selected option', () => {
    const onChange = vi.fn();
    render(<SeriesMonitoringOptionsPopover value="all" onChange={onChange} />);
  ...
  it('displays the correct label for each monitoring option', () => {
    const onChange = vi.fn();
    render(<SeriesMonitoringOptionsPopover value="future" onChange={onChange} />);
  ...
  it('closes popover when clicking outside', () => {
    ...
    render(
      <div>
        <SeriesMonitoringOptionsPopover value="all" onChange={onChange} />
  ...
  it('disables button when disabled prop is true', () => {
    const onChange = vi.fn();
    render(<SeriesMonitoringOptionsPopover value="all" onChange={onChange} disabled />);
  ...
  it('shows aria-selected for the selected option', () => {
    const onChange = vi.fn();
    render(<SeriesMonitoringOptionsPopover value="none" onChange={onChange} />);
  ...
  it.each<MonitoringOption>(['all', 'future', 'missing', 'existing', 'pilot', 'firstSeason', 'none'])(
    'displays correct label for %s option',
    option => {
      const onChange = vi.fn();
      const config = MONITORING_OPTIONS.find(o => o.value === option);
      render(<SeriesMonitoringOptionsPopover value={option} onChange={onChange} />);
