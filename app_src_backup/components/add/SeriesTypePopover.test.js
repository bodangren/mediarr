...
import { SeriesTypePopover, SERIES_TYPE_OPTIONS } from './SeriesTypePopover';
describe('SeriesTypePopover', () => {
    it('renders with default selected option', () => {
        const onChange = vi.fn();
        render(_jsx(SeriesTypePopover, { value: "standard", onChange: onChange }));
        expect(screen.getByText('Standard')).toBeInTheDocument();
    });
    it('displays the correct label for each series type', () => {
        const onChange = vi.fn();
        render(_jsx(SeriesTypePopover, { value: "anime", onChange: onChange }));
    ...
    it('closes popover when clicking outside', () => {
        const onChange = vi.fn();
        render(_jsxs("div", { children: [_jsx(SeriesTypePopover, { value: "standard", onChange: onChange }), _jsx("div", { "data-testid": "outside", children: "Outside" })] }));
        fireEvent.click(screen.getByRole('button', { name: /standard/i }));
    ...
    it('disables button when disabled prop is true', () => {
        const onChange = vi.fn();
        render(_jsx(SeriesTypePopover, { value: "standard", onChange: onChange, disabled: true }));
    ...
    it('shows aria-selected for the selected option', () => {
        const onChange = vi.fn();
        render(_jsx(SeriesTypePopover, { value: "daily", onChange: onChange }));
    ...
    it.each(['standard', 'anime', 'daily'])('displays correct label for %s option', option => {
        const onChange = vi.fn();
        const config = SERIES_TYPE_OPTIONS.find(o => o.value === option);
        render(_jsx(SeriesTypePopover, { value: option, onChange: onChange }));
        expect(screen.getByText(config.label)).toBeInTheDocument();
    });
});
//# sourceMappingURL=SeriesTypePopover.test.js.map