...
describe('PageSidebar with grouped navigation', () => {
    it('renders navigation sections with headers', () => {
        render(_jsx(PageSidebar, { pathname: "/indexers", collapsed: false, onToggle: vi.fn() }));
    ...
    it('shows only icons when sidebar is collapsed', () => {
        render(_jsx(PageSidebar, { pathname: "/indexers", collapsed: true, onToggle: vi.fn() }));
        // Should show short labels (or icons only)
        expect(screen.getByText('Series')).toBeInTheDocument();
        expect(screen.getByText('Movies')).toBeInTheDocument();
    });
});
//# sourceMappingURL=PageSidebar.test.js.map