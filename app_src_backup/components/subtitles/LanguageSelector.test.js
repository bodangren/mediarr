...
describe('LanguageSelector', () => {
    it('renders with label and select button', () => {
        render(_jsx(LanguageSelector, { value: "en", onChange: vi.fn(), label: "Language", id: "language-select" }));
        expect(screen.getByLabelText('Language')).toBeInTheDocument();
        expect(screen.getByRole('button', { name: 'Language' })).toBeInTheDocument();
        expect(screen.getByText('English (en)')).toBeInTheDocument();
    });
    it('displays selected language correctly', () => {
        render(_jsx(LanguageSelector, { value: "fr", onChange: vi.fn() }));
        expect(screen.getByText('French (fr)')).toBeInTheDocument();
    });
    it('displays placeholder when no language is selected', () => {
        render(_jsx(LanguageSelector, { value: "", onChange: vi.fn() }));
        expect(screen.getByText('Select language...')).toBeInTheDocument();
    });
    it('opens dropdown when button is clicked', () => {
        render(_jsx(LanguageSelector, { value: "en", onChange: vi.fn() }));
    ...
    it('calls onChange when language is selected', () => {
        ...
        render(_jsx(LanguageSelector, { value: "", onChange: handleChange }));
        const button = screen.getByRole('button', { name: 'Select language' });
        fireEvent.click(button);
    ...
    it('filters languages by exclude prop', () => {
        render(_jsx(LanguageSelector, { value: "", onChange: vi.fn(), exclude: ['en', 'es'] }));
        const button = screen.getByRole('button', { name: 'Select language' });
        fireEvent.click(button);
    ...
    it('is disabled when disabled prop is true', () => {
        render(_jsx(LanguageSelector, { value: "en", onChange: vi.fn(), disabled: true }));
        const button = screen.getByRole('button', { name: 'Select language' });
        expect(button).toBeDisabled();
    });
    it('applies custom className', () => {
        render(_jsx(LanguageSelector, { value: "en", onChange: vi.fn(), className: "custom-class" }));
