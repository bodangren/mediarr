...
describe('MovieActionsToolbar', () => {
    ...
    it('renders all action buttons', () => {
        render(_jsx(MovieActionsToolbar, { ...mockHandlers }));
    ...
    it('adds spinning animation to refresh icon when syncing', () => {
        const { container } = render(_jsx(MovieActionsToolbar, { ...mockHandlers, isSyncing: true }));
        const syncButton = screen.getByRole('button', { name: /sync movie/i });
        expect(syncButton).toContainHTML('animate-spin');
    });
    it('adds spinning animation to disk icon when scanning', () => {
        const { container } = render(_jsx(MovieActionsToolbar, { ...mockHandlers, isScanning: true }));
        const scanButton = screen.getByRole('button', { name: /scan disk/i });
        expect(scanButton).toContainHTML('animate-spin');
    });
    it('adds spinning animation to search icon when searching', () => {
        const { container } = render(_jsx(MovieActionsToolbar, { ...mockHandlers, isSearching: true }));
        const searchButton = screen.getByRole('button', { name: /search all/i });
        expect(searchButton).toContainHTML('animate-spin');
    });
    it('does not disable buttons when loading states are false', () => {
        render(_jsx(MovieActionsToolbar, { ...mockHandlers, isSyncing: false, isScanning: false, isSearching: false }));
    ...
    it('has correct accessibility attributes', () => {
        ...
        expect(screen.getByRole('button', { name: /search all/i })).toHaveAttribute('aria-label', 'Search all subtitles');
        expect(screen.getByRole('button', { name: /manual search/i })).toHaveAttribute('aria-label', 'Manual search');
        expect(screen.getByRole('button', { name: /upload/i })).toHaveAttribute('aria-label', 'Upload subtitles');
    });
    it('applies correct button styles', () => {
        const { container } = render(_jsx(MovieActionsToolbar, { ...mockHandlers }));
        const buttons = container.querySelectorAll('button');
        buttons.forEach(button => {
            expect(button).toHaveClass('rounded-sm', 'border', 'border-border-subtle', 'px-3', 'py-1.5', 'text-sm', 'transition-colors');
        });
    });
});
//# sourceMappingURL=MovieActionsToolbar.test.js.map