...
describe('ProviderSettingsModal', () => {
    ...
    const mockOpenSubtitlesProvider = {
        id: 'opensubtitles-1',
        name: 'OpenSubtitles',
        enabled: true,
        type: 'opensubtitles',
    ...
    it('does not render when provider is null', () => {
        ...
        const { container } = render(_jsx(ProviderSettingsModal, { isOpen: true, provider: null, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
        expect(container.firstChild).toBeNull();
    });
    it('renders OpenSubtitles provider fields', () => {
        const mockOnClose = vi.fn();
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockOpenSubtitlesProvider, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
    ...
    it('renders Subscene provider with info message', () => {
        const mockOnClose = vi.fn();
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockSubsceneProvider, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
    ...
    it('renders Addic7ed provider fields', () => {
        const mockOnClose = vi.fn();
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockAddic7edProvider, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
        expect(screen.getByLabelText('Username')).toBeInTheDocument();
        expect(screen.getByLabelText('Password')).toBeInTheDocument();
    });
    it('renders Generic provider fields', () => {
        const mockOnClose = vi.fn();
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockGenericProvider, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
    ...
    it('calls onReset on second click of Reset button', async () => {
        ...
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockGenericProvider, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset }));
        const resetButton = screen.getByRole('button', { name: 'Reset' });
        fireEvent.click(resetButton);
        fireEvent.click(resetButton);
        await waitFor(() => {
    ...
    it('disables all action buttons while saving', () => {
        const mockOnClose = vi.fn();
        render(_jsx(ProviderSettingsModal, { isOpen: true, provider: mockSubsceneProvider, isSaving: true, onSave: mockOnSave, onTest: mockOnTest, onReset: mockOnReset, onClose: mockOnClose }));
...
//# sourceMappingURL=ProviderSettingsModal.test.js.map