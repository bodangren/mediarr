...
describe('SubtitleProgressBar', () => {
    it('renders progress bar correctly', () => {
        render(_jsx(SubtitleProgressBar, { total: 50, complete: 45 }));
    ...
    it('calculates percentage correctly', () => {
        render(_jsx(SubtitleProgressBar, { total: 100, complete: 75 }));
        expect(screen.getByText('75% complete')).toBeInTheDocument();
    });
    it('shows green color for >90% completion', () => {
        const { container } = render(_jsx(SubtitleProgressBar, { total: 100, complete: 95 }));
        const progressBar = container.querySelector('.bg-accent-success');
        expect(progressBar).toBeInTheDocument();
    });
    it('shows yellow color for 50-90% completion', () => {
        const { container } = render(_jsx(SubtitleProgressBar, { total: 100, complete: 70 }));
        const progressBar = container.querySelector('.bg-accent-warning');
        expect(progressBar).toBeInTheDocument();
    });
    it('shows red color for <50% completion', () => {
        const { container } = render(_jsx(SubtitleProgressBar, { total: 100, complete: 30 }));
        const progressBar = container.querySelector('.bg-accent-danger');
        expect(progressBar).toBeInTheDocument();
    });
    it('renders label when provided', () => {
        render(_jsx(SubtitleProgressBar, { total: 50, complete: 45, label: "Episode Subtitles" }));
        expect(screen.getByText('Episode Subtitles')).toBeInTheDocument();
    });
    it('does not render label when not provided', () => {
        const { container } = render(_jsx(SubtitleProgressBar, { total: 50, complete: 45 }));
    ...
    it('handles zero total', () => {
        render(_jsx(SubtitleProgressBar, { total: 0, complete: 0 }));
        const zeros = screen.getAllByText('0');
        expect(zeros.length).toBeGreaterThan(0);
        expect(screen.getByText('0% complete')).toBeInTheDocument();
    });
    it('handles zero complete', () => {
        render(_jsx(SubtitleProgressBar, { total: 50, complete: 0 }));
    ...
    it('handles full completion', () => {
        render(_jsx(SubtitleProgressBar, { total: 50, complete: 50 }));
    ...
    it('clamps percentage to 100%', () => {
        render(_jsx(SubtitleProgressBar, { total: 10, complete: 15 }));
        expect(screen.getByText('100% complete')).toBeInTheDocument();
    });
    it('clamps percentage to 0%', () => {
        render(_jsx(SubtitleProgressBar, { total: 10, complete: -5 }));
        expect(screen.getByText('0% complete')).toBeInTheDocument();
    });
    it('applies custom className', () => {
        render(_jsx(SubtitleProgressBar, { total: 50, complete: 45, className: "custom-class" }));
        const wrapper = document.querySelector('.custom-class');
        expect(wrapper).toBeInTheDocument();
    });
    it('shows transition animation on progress bar', () => {
        const { container } = render(_jsx(SubtitleProgressBar, { total: 100, complete: 50 }));
        const progressBar = container.querySelector('[style*="width: 50%"]');
        expect(progressBar).toHaveClass('transition-all', 'duration-300');
    });
    it('handles fractional progress', () => {
        render(_jsx(SubtitleProgressBar, { total: 3, complete: 1 }));
        expect(screen.getByText('33% complete')).toBeInTheDocument();
    });
    it('displays missing count correctly', () => {
        render(_jsx(SubtitleProgressBar, { total: 100, complete: 60 }));
        expect(screen.getByText('40 missing')).toBeInTheDocument();
    });
    it('rounds percentage correctly', () => {
        render(_jsx(SubtitleProgressBar, { total: 3, complete: 2 }));
        expect(screen.getByText('67% complete')).toBeInTheDocument();
    });
});
//# sourceMappingURL=SubtitleProgressBar.test.js.map