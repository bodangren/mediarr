...
describe('SubtitleTrackList', () => {
    it('renders available subtitles section', () => {
        render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn(), onDelete: vi.fn() }));
        expect(screen.getByText('Available Subtitles')).toBeInTheDocument();
        expect(screen.getByText('en')).toBeInTheDocument();
        expect(screen.getByText('es')).toBeInTheDocument();
    });
    it('renders missing languages section', () => {
        render(_jsx(SubtitleTrackList, { tracks: [], missingLanguages: mockMissingLanguages, onSearch: vi.fn(), onDelete: vi.fn() }));
    ...
    it('calls onDelete when delete button is clicked', () => {
        ...
        render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn(), onDelete: handleDelete }));
    ...
    it('calls onSearch when missing language badge is clicked', () => {
        ...
        render(_jsx(SubtitleTrackList, { tracks: [], missingLanguages: mockMissingLanguages, onSearch: handleSearch, onDelete: vi.fn() }));
        fireEvent.click(screen.getByText('fr'));
        expect(handleSearch).toHaveBeenCalledWith('fr');
    });
    it('renders download button when onDownload is provided', () => {
        render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn(), onDelete: vi.fn(), onDownload: vi.fn() }));
    ...
    it('does not render delete button when onDelete is not provided', () => {
        render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn() }));
    ...
    it('renders empty state when no tracks or missing languages', () => {
        render(_jsx(SubtitleTrackList, { tracks: [], missingLanguages: [], onSearch: vi.fn(), onDelete: vi.fn() }));
    ...
    it('applies custom className', () => {
        const { container } = render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn(), onDelete: vi.fn(), className: "custom-class" }));
    ...
    it('disables delete button when deleting', () => {
        const handleDelete = vi.fn();
        const { rerender } = render(_jsx(SubtitleTrackList, { tracks: mockTracks, missingLanguages: [], onSearch: vi.fn(), onDelete: handleDelete }));
    ...
    it('truncates long file paths correctly', () => {
        ...
        render(_jsx(SubtitleTrackList, { tracks: [longPathTrack], missingLanguages: [], onSearch: vi.fn(), onDelete: vi.fn() }));
        const pathElement = screen.getByTitle(longPathTrack.path);
        expect(pathElement.textContent).toContain('...');
    });
});
//# sourceMappingURL=SubtitleTrackList.test.js.map