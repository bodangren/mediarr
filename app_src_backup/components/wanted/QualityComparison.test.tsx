...
describe('QualityComparison', () => {
  it('renders current and cutoff quality with arrow', () => {
    render(<QualityComparison current="720p" cutoff="1080p" />);

    expect(screen.getByText('720p')).toBeInTheDocument();
    expect(screen.getByText('1080p')).toBeInTheDocument();
    // Arrow icon should be present in the document (hidden from accessibility but in DOM)
    const { container } = render(<QualityComparison current="720p" cutoff="1080p" />);
    expect(container.querySelector('svg')).toBeInTheDocument();
  });

  it('renders with custom className', () => {
    const { container } = render(
      <QualityComparison current="Bluray-720p" cutoff="Bluray-1080p" className="text-red-500" />,
    );
  ...
  it('renders complex quality strings', () => {
    render(<QualityComparison current="WEB-DL-1080p" cutoff="Bluray-2160p" />);
