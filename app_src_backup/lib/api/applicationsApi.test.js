...
describe('applicationsApi', () => {
    ...
    const api = createApplicationsApi(mockClient);
    it('lists applications', async () => {
        mockClient.request.mockResolvedValue([]);
        await api.list();
        expect(mockClient.request).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/applications' }), expect.anything());
    });
    ...
    it('syncs all via /api/applications/sync', async () => {
        ...
        await api.syncAll();
        expect(mockClient.request).toHaveBeenCalledWith(expect.objectContaining({ path: '/api/applications/sync', method: 'POST' }), expect.anything());
    });
});
//# sourceMappingURL=applicationsApi.test.js.map