...
import { apiErrorEnvelopeSchema, paginatedEnvelopeSchema, successEnvelopeSchema, } from './schemas';
...
export class ApiHttpClient {
    ...
    async requestPaginated(config, itemSchema) {
        const payload = await this.execute(config);
        const errorEnvelope = apiErrorEnvelopeSchema.safeParse(payload.body);
        if (errorEnvelope.success) {
            throw new ApiClientError({
        ...
        if (!parsed.success) {
            throw new ContractViolationError('Response did not match paginated envelope contract', {
                issues: parsed.error.issues,
                payload: payload.body,
            });
        }
    ...
    executeWithXhr(requestUrl, config) {
        return new Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
                ...
                if (!contentType.toLowerCase().includes('application/json')) {
                    reject(new ContractViolationError('Expected JSON response payload', {
                        status: xhr.status,
                        contentType,
                    }));
                    return;
                ...
                try {
                    ...
                    reject(new ContractViolationError('Failed to parse JSON response payload', {
                        status: xhr.status,
                    }));
                    return;
                }
