...
describe('useTouchGestures', () => {
    ...
    it('detects left swipe gesture', () => {
        ...
        const { result } = renderHook(() => useTouchGestures(mockElement, {
            onSwipeLeft,
            threshold: 50,
        }));
        act(() => {
    ...
    it('does not trigger swipe below threshold', () => {
        ...
        act(() => {
            ...
            mockElement.dispatchEvent(touchMoveEvent);
            const touchEndEvent = new TouchEvent('touchend', {
                changedTouches: [{ clientX: 75, clientY: 50 }],
                bubbles: true,
                cancelable: true,
            });
            mockElement.dispatchEvent(touchEndEvent);
    ...
    it('ignores vertical swipes (not configured)', () => {
        ...
        const onSwipeLeft = vi.fn();
        const onSwipeRight = vi.fn();
        renderHook(() => useTouchGestures(mockElement, {
            onSwipeLeft,
            onSwipeRight,
            threshold: 50,
        }));
    ...
    it('handles diagonal swipes correctly based on dominant direction', () => {
        ...
        act(() => {
            ...
            mockElement.dispatchEvent(touchStartEvent);
            const touchMoveEvent = new TouchEvent('touchmove', {
                touches: [{ clientX: 40, clientY: 70 }], // Moves left and slightly down
                bubbles: true,
                cancelable: true,
            });
            mockElement.dispatchEvent(touchMoveEvent);
            const touchEndEvent = new TouchEvent('touchend', {
                changedTouches: [{ clientX: 30, clientY: 75 }],
                bubbles: true,
                cancelable: true,
            });
            mockElement.dispatchEvent(touchEndEvent);
    ...
    it('supports custom threshold values', () => {
        ...
        act(() => {
            ...
            mockElement.dispatchEvent(touchStartEvent);
            const touchMoveEvent = new TouchEvent('touchmove', {
                touches: [{ clientX: 60, clientY: 50 }], // Only 60px movement
                bubbles: true,
                cancelable: true,
            });
            mockElement.dispatchEvent(touchMoveEvent);
...
//# sourceMappingURL=useTouchGestures.test.js.map