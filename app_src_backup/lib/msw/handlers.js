...
import { createMockDataset, paginate, } from './factories';
function numberQuery(url, key, fallback) {
...
export function createHandlers(mode = 'deterministic') {
    ...
    return [
        ...
        http.post('/api/media/search', async ({ request }) => {
            ...
            if (body.mediaType.toUpperCase() === 'MOVIE') {
                return sendSuccess(dataset.movies
            ...
            return sendSuccess(dataset.series
        ...
        http.post('/api/indexers', async ({ request }) => {
            ...
            const created = {
                ...
                implementation: typeof body.implementation === 'string' ? body.implementation : 'Torznab',
                configContract: typeof body.configContract === 'string'
                    ? body.configContract
                    : 'TorznabSettings',
        ...
        http.post('/api/indexers/test', async ({ request }) => {
            ...
            if (typeof body.settings === 'string') {
                try {
                    parsedSettings = JSON.parse(body.settings);
                }
                catch {
