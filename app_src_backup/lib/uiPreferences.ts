...
export function loadUIPreferences(storage?: StorageLike): UIPreferences {
  ...
  try {
    ...
    return {
      ...
      showRelativeDates:
        typeof parsed.showRelativeDates === 'boolean'
          ? parsed.showRelativeDates
          : DEFAULT_UI_PREFERENCES.showRelativeDates,
      colorImpairedMode:
        typeof parsed.colorImpairedMode === 'boolean'
...
export function saveUIPreferences(preferences: UIPreferences, storage?: StorageLike): void {
  const resolvedStorage = resolveStorage(storage);
  if (!resolvedStorage) {
    return;
  }
...
function resolveTheme(theme: ThemePreference, doc: Document): 'light' | 'dark' {
...
export function applyUIPreferences(preferences: UIPreferences, doc?: Document): void {
  if (!doc) {
