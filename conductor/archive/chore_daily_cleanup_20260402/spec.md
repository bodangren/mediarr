...
1. **Staged deletions of already-archived track folders** — `bug_mediasearch_corner_cases_20260401/` and `chore_cleanup_uncommitted_work_20260401/` were moved to `conductor/archive/` but git still tracks the source deletions.
2. **`.env` file modified** — must not be committed (contains secrets).
3. **A stale artifact file** — `conductor/archive/cardigann_runtime_parity_20260223/artifacts/final-phase5-compatibility-matrix.json` has uncommitted changes.
