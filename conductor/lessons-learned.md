...
- (2026-04-15, feature_flutter_search_add) **Flutter widget tests with FutureProvider require ProviderScope overrides.** Use `searchResultsProvider.overrideWith((ref) async => [...])` pattern to control async state. StateNotifierProvider requires `ref.read(provider.notifier)` access in overrides.
