...
- **Cross-Platform Client:** A Flutter-based client targeting Android TV, Linux desktop, and macOS. Features a 10-foot leanback UI for browsing the library and playing media on the big screen or desktop, with automatic local network discovery (mDNS), robust 4K/HDR playback, and seamless playback state syncing with the server.