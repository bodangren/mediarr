...
| 2026-04-04 | bug_organize_services_corner_cases_20260404 | MovieOrganizeService `{AudioChannels}` token never populated from variant data | Low | Open | Media analysis not wired to variant; token always resolves to empty string |
| 2026-04-04 | bug_organize_services_corner_cases_20260404 | SeriesOrganizeService `applyRename` can leave partial state if DB update fails after fs.rename | Medium | Open | File is moved on disk but DB path not updated; no rollback mechanism exists |
