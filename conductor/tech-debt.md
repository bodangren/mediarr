...
| 2026-03-31 | bug_corner_case_testing_20260331 | ImportManager parsed-episode fallback to movie path broken | High | Open | `ImportManager.ts:439` guards movie path with `if (!parsed)` — episode parsed but no DB match goes to "no match" instead of movie path |
