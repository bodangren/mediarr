...
- [ ] Task: Create `HomeScreen` — vertical scrollable layout with section headers and horizontal card rows
- [ ] Task: Implement Continue Watching section — call `GET /api/playback/continue-watching` via ApiClient; render as horizontal `ListView` of cards with progress bar, poster, title; one-tap navigates to `PlaybackScreen` with resume offset
- [ ] Task: Implement Recently Added section — call `GET /api/activity?types=download,import&limit=10`; render as horizontal card row
- [ ] Task: Implement Upcoming section — call `GET /api/dashboard/upcoming`; render as horizontal card row with air date badge
- [ ] Task: Set `HomeScreen` as default route in `app_router.dart`; update `LeanbackScaffold` nav to include Home
- [ ] Task: Write tests for `HomeScreen` — renders all 3 sections, handles empty states, cards are tappable
