...
- [x] Adjust selectors in `InteractiveSearchModal`, `MovieInteractiveSearchModal`, `SeriesInteractiveSearchModal`, `EditCollectionModal`, and `PageLayout` tests if markup changed. (`6b4dfcd3`, `77533167`)
  ...
  - Evidence: Follow-up stabilization `77533167` increased the `SeriesInteractiveSearchModal` "closes on Escape key press" test timeout to 10s to eliminate flaky 5s timeout failures under heavy parallel render load.
