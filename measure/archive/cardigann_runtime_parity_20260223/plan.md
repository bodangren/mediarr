...
- [x] Task: Measure - User Manual Verification 'Phase 1' (Protocol in workflow.md) e6dd9e1
...
- [x] Task: Measure - User Manual Verification 'Phase 2' (Protocol in workflow.md) efbd472
...
- [x] Task: Implement Search Inputs, Headers, and Path Inheritance
    - [x] Sub-task: Write tests - `search.inputs` and per-path `inputs` application.
    - [x] Sub-task: Write tests - `search.headers` and request header emission.
    - [x] Sub-task: Write tests - inherited path/input behavior where used.
    - [x] Sub-task: Implement request builder updates for these features.

- [x] Task: Implement Response-Type and Row Parsing Behaviors
    - [x] Sub-task: Write tests - HTML row selector extraction with required row options.
    - [x] Sub-task: Write tests - JSON response extraction path used by definitions.
    - [x] Sub-task: Write tests - category mapping and normalization behavior.
    - [x] Sub-task: Implement parser/runtime support for required row/response semantics.
...
- [x] Task: Enforce Definition Compatibility Validation
    - [x] Sub-task: Write tests - incompatible definitions are rejected with clear diagnostics.
    - [x] Sub-task: Wire loader/factory validation to runtime initialization paths.
    - [x] Sub-task: Ensure diagnostics include definition ID, feature, and remediation hint.

- [x] Task: Protect Cardigann UI/API Round-Trip
    - [x] Sub-task: Write tests - edit/save preserves `definitionId` and definition-specific fields.
    - [x] Sub-task: Write tests - schema lookup for `CardigannSettings` in add/edit paths.
    - [x] Sub-task: Implement UI/backend schema resolution and payload safeguards.

- [x] Task: Final Conformance Gate and Coverage
    - [x] Sub-task: Run full Cardigann conformance harness and capture final matrix.
    - [x] Sub-task: Ensure required imported definitions are either green or explicitly blocked with diagnostics.
    - [x] Sub-task: Validate coverage target for Cardigann runtime changes.
    - [x] Sub-task: Document remaining out-of-scope Cardigann features for future track.
