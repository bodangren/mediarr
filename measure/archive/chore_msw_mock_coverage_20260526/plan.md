...
- [x] Run `CI=true npm test` — **Pre-existing failures confirmed**: 15 failures across 8 test files, all pre-existing at base commit before this track. No MSW handler test file (`handlers.s1/s2/s3/s4/s5/s6.test.ts`) appears in failures. Failures: `api-route-map.test.ts` (Zod import), `TorrentManager.test.ts` (BigInt mixing), `closeDrizzleMigration.s4.shimRemotion.test.ts` (PrismaClient references), `VariantSubtitleFetchService`/`VariantBackfillService` (BigInt), `BulkImportService` (undefined drizzle mock), `media-repository.test.js` (undefined drizzle insert), subtitle variant tests, `episode-service.test.js`/`series-service.test.js` (deleted services). The S6 targeted command passes 9/9; S1+S2+S3+S4+S5+S6 co-run passes 265/265. This track's Green gate is satisfied. *(commit `b14337b`)*
- [x] Verify no unhandled MSW warnings in test output *(Red test: source assertion on `app/src/test/setup.ts` for the MSW lifecycle hook with `onUnhandledRequest: 'error'` — wired in Green phase: imports server from `@/lib/msw/server`, `beforeAll` calls `server.listen({ onUnhandledRequest: 'error' })`, `afterEach` calls `server.resetHandlers()`, `afterAll` calls `server.close()`)* *(commit `b14337b`)*
- [x] Update `tech-debt.md` — mark "MSW mock coverage incomplete" as Resolved *(Red test: markdown assertion on `measure/tech-debt.md` for the `chore_msw_mock_coverage_20260526` row's Status column — row updated from `Open` to `Resolved` in Green phase)* *(commit `b14337b`)*
- [x] Final commit and push *(commit `b14337b` — final S6 verification commit includes setup.ts MSW hooks, test regex fix, tech-debt.md update, and plan.md evidence)*

> **Green phase (2026-06-12, JR):** Targeted S6 command passes 9/9 at `b14337b`. S1+S2+S3+S4+S5+S6 co-run passes 265/265 — no regressions. Test regex fix: `afterEach`/`afterAll` regexes had a structural bug (`[^)]*\{` can never match valid JS because `)` in `() =>` stops `[^)]*` before `{`); fixed to `[^)]*\)[^{]*\{`. `graph.db` updated for `setup.ts` (2 import edges: vitest, jest-dom). `tech-debt.md` row updated to Resolved.
>
> **Green gate note (full suite):** `npm test` (full suite via `bun node_modules/.bin/vitest run`) exits with code 1 due to **pre-existing failures** unrelated to this track.
> These same failures were documented in the S1–S5 green gate notes.
> No MSW handler test file (`handlers.s1/s2/s3/s4/s5/s6.test.ts`) appears in the failures.
> The S6 targeted command passes **9/9**. S1+S2+S3+S4+S5+S6 co-run passes **265/265**.
> Pre-existing failures (15 tests across 8 files): `api-route-map.test.ts` (Zod import), `TorrentManager.test.ts` (BigInt/number mixing), `VariantBackfillService`/`VariantSubtitleFetchService` (BigInt/number), `BulkImportService` (undefined drizzle mock), `closeDrizzleMigration.s4.shimRemotion.test.ts` (PrismaClient references), `media-repository.test.js` (undefined drizzle insert), `episode-service.test.js`/`series-service.test.js` (deleted modules), subtitle variant tests (BigInt). This track's Green gate is satisfied — the targeted and co-run commands are the authoritative gate.
