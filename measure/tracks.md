...
- [~] **Track: MSW Mock Coverage for Backend Routes** *(deferred — merge candidate)* *Link: [./tracks/chore_msw_mock_coverage_20260526/](./tracks/chore_msw_mock_coverage_20260526/)* — Only valuable once consuming integration tests exist; pair with the component-gaps track. S1 (core domains, 35 tests), S2 (settings/quality/download-client, 31 tests), S3 (system/operations/stats, 36 tests), S4 (subtitle/playback, 40 tests), S5 (remaining domains, 114 tests) all complete and GREEN. 256 total MSW handler tests across 5 suites. Full suite has pre-existing failures only (no MSW handler failures). **S6 Red phase complete (commit `3f2b3c4`, 9 tests, 6 Red failures)**: 6 contract assertions for the MSW setup hook (`app/src/test/setup.ts` wiring with `onUnhandledRequest: 'error'` + MSW lifecycle hooks) and 3 for the tech-debt.md update; fails at HEAD because setup.ts has no MSW wiring and the tech-debt.md row is `Open`. Awaiting Green phase to wire the setup hook and mark tech-debt.md Resolved.
