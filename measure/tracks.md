...
1. - [~] **Track: Home Network Deployment Hardening** *Phases: 6 | Link: [./tracks/chore_home_network_deployment_hardening_20260712/](./tracks/chore_home_network_deployment_hardening_20260712/)* — 18/20 tasks done. **Blocker (a) is STILL OPEN**, but the diagnosis is now much sharper and four hypotheses are positively ruled out by probe. The defect is **intermittent** (3 no-cache builds: fail → pass → fail on the *same* Dockerfile, with a different module each time). Probes disproved the missing-dependency, npm-flag, disk-pressure, and Vite/source explanations: the install is correct in-image, and the same build run from the *committed* layer succeeds (3028 modules). **None of the four documented remediation patterns applies** — all assume an install-layout problem that does not exist. Current hypothesis is overlay write-visibility under rootless podman between `RUN` steps; next experiment is `--layers=false` and `--storage-driver=vfs`. The old Red gate was *passing* while pinning the broken combined-layer shape; it now asserts the split-layer invariant. **Blocker (b) stands:** Docker Engine + physical Android/LAN playback verification is human-gated because this host's `docker` is Podman 4.9.3.
