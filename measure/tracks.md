...
1. - [~] **Track: AI Release Parser Lockdown** *Phases: 6 | Link: [./tracks/bug_ai_release_parser_lockdown_20260728/](./tracks/bug_ai_release_parser_lockdown_20260728/)* — Created 2026-07-28. **The AI release parser has never worked in its shipped configuration.** `ReleaseParserProvider.ts:5` defaults to `minimax/minimax-m2.7`, measured live at **77–88s per call** against `parse()`'s 15s and `parseBatch()`'s 20s abort deadlines — 4–6× slower than its own timeout, so every call aborts, `parse()` silently returns the 36-line regex fallback and `parseBatch()` returns `[]`. `.env` on this host sets only `OPENROUTER_API_KEY`, so the hardcoded default *is* the live path. The `.env.example` gateway default (`openrouter/free`) has the same defect at 31–35s, and the gateway it points at is not running. **Second, independent defect:** `MediaSearchService.ts:580` zips `parseBatch` results onto releases **by array position**, so a truncated response (reproduced live on two models — 7-of-8 and 6-of-8) shifts every subsequent parse onto the wrong release; `relevanceScore` is then used directly as `confidenceScore` and auto-grab fires at `totalScore >= 50`, so a mis-attributed parse can trigger an automatic download of the wrong release. `parseFiles()` already carries the length guard `parseBatch()` lacks. Fix: golden set → index-based batch alignment → configurable timeouts → pin `google/gemini-2.5-flash-lite` (2.4–3.1s, $0.047/1000 titles, byte-identical across 3 runs at temp=0) → live accuracy benchmark. Also replaces `parse()`'s serial queue (25 candidates ≈ 20 min stall in `WantedSearchService`) with bounded concurrency, and claims the `ReleaseParserProvider` coverage gap from tech-debt.
2. - [~] **Track: Home Network Deployment Hardening** *Phases: 6 | Link: [./tracks/chore_home_network_deployment_hardening_20260712/](./tracks/chore_home_network_deployment_hardening_20260712/)* — 18/20 tasks done. **Blocker (a) is STILL OPEN and the mechanism is UNKNOWN.** Reconciled 2026-07-27 (later session) after an instrumented investigation across 21 builds. The previously recorded root cause — overlay write-visibility between `RUN` layers — is **disproven**, along with missing/unhoisted deps, the `npm ci` flags, Node version, npm version, fd exhaustion, disk pressure, and memory pressure. A probe placed *inside the failing build layer* resolves all 47 `app` deps from `/app/app` immediately before rollup fails and again after, with all 37 `@radix-ui` packages complete; `tsc -b` resolves the same specifier seconds earlier in the same command. So the files are present and resolvable at the moment rollup says they are not. The defect reproduced twice (17:19, 17:27; 132 and 143 modules, different module each) and then went **13 consecutive green clean builds**, including a 3/3 true baseline — it is **not reproducible on demand**. Two arms (Node 22, npm 10.9.8) looked like fixes until a control changing only layer structure went 3/3 green and killed both. **Instrumentation landed 2026-07-28** — all three recommended follow-ups are done and the mechanism is still unknown, but the next occurrence will now diagnose itself: `scripts/docker-build-spa.sh` re-runs a failed SPA build under `DEBUG=vite:resolve`, probes `require.resolve` for the unresolved specifier, prints the trace, and exits with the *original* status (harness-verified on all three arms, so an intermittent failure cannot be masked by the retry); the 7–13 min `docker build --no-cache` is now gated behind `CLEAN_IMAGE_BUILD_TESTS=true` / `npm run test:clean-image` instead of running inside every `vitest run`; and `clean-workspace-invariant.test.js` no longer pins the Dockerfile shape (`toEqual` on the `RUN npm` list is gone) — it parses logical instructions and asserts only "one frozen install, one SPA build, different `RUN` layers, install first", mutation-tested both ways. **The instrumentation fired on its first live run and overturned a documented exclusion:** the defect is reproducing again (3 of 4 first attempts failed 2026-07-28, at 118/119/130 modules — every observed failure now sits in a 118–143 module band), and the environment block printed from inside the failing `RUN` layer shows **`Max open files` 1024/1024, not the 1048576 that was measured on 2026-07-27 under `podman run`** — a different namespace from the one the build actually enters. Independently confirmed: buildah `RUN` = 1024, `podman run` = 1048576, host = 1048576. **Fd exhaustion is back on the table.** Sharpest single observation: a failing build and a clean 3028-module build in the *same* `RUN` layer seconds apart, which no install-layout or filesystem explanation survives. Mechanism still unproven (no `EMFILE` observed); next arm is `build.rollupOptions.maxParallelFileOps` in `app/vite.config.ts` (portable, preferred) with `--ulimit nofile=65536:65536` as the diagnostic arm — the layer cannot raise its own limit, since rootless podman drops `CAP_SYS_RESOURCE`. **Blocker (b) stands:** Docker Engine + physical Android/LAN playback verification is human-gated because this host's `docker` is Podman 4.9.3.
3. - [~] **Track: Database Migration and Data Preservation Strategy** *Phases: 5 | Link: [./tracks/database_migration_strategy_20260713/](./tracks/database_migration_strategy_20260713/)* — 21/25 tasks done; the 4 open are per-phase *User Manual Verification* steps, which are human-gated. Delivered `server/src/db/migrationRunner.ts`, wired into `startApi()` and the container entrypoint, plus `docs/migration-runbook.md`. **Found and fixed the actual mechanism behind the "existing data lost" tech-debt row:** drizzle-orm applies all pending migrations in one transaction, SQLite ignores `PRAGMA foreign_keys` inside a transaction, and `DROP TABLE Series` during the `0001` table rebuild cascade-deletes every `Season` and `Episode` row **while committing successfully**. Proven by a Red test (`Season: 1 -> 0, Episode: 1 -> 0`), now green.
4. - [~] **Track: Flutter Media Detail Page** *Phases: 5 | Link: [./tracks/feature_flutter_media_detail_20260508/](./tracks/feature_flutter_media_detail_20260508/)* — 33/35 tasks done. The "commit and push of ~76 local commits" task **was stale**: `main` is level with `origin/main`, so that backlog no longer exists. **The claim that "no agent-executable work remains" was wrong** — preparing the movie-detail smoke protocol found that the **Delete action was a silent no-op** (`onPressed: () {}`, with a false comment claiming the server had no DELETE endpoint; `DELETE /api/movies/:id` has existed all along), and the widget test that supposedly covered it never asserted any API call, so it passed green against the placeholder. Both are now fixed and genuinely covered. Only on-device visual confirmation remains human-gated.
...
- [ ] **Track: Remaining Server Service Test Coverage** *Phases: 6 | Link: [./tracks/chore_remaining_server_service_coverage_20260728/](./tracks/chore_remaining_server_service_coverage_20260728/)* — Pending. Closes the last verified server-service coverage gap from tech-debt.md (2026-07-26): 8 of 55 services lack a sibling `.test.ts` (`ActivityEventEmitter`, `DataDirectoryInitializer`, `LibraryScanner`, `MetadataGenerator`, `MetadataProvider`, `ProbeMetadataParser`, `ReleaseParserProvider`, `WantedService`), re-verified against source 2026-07-28. TDD, ≥80% branch coverage per service; no production changes unless a defect is found.
