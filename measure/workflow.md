...
4. **High Code Coverage:** Aim for >80% code coverage for all modules, 100% for all APIs
...
5.  **Attach Auditable Verification Report using Git Notes:**
    -   **Step 5.1: Draft Note Content:** Create a detailed verification report including the automated test command and the outcome.
...
3. **Move the Track Directory**
   ```bash
   mv measure/tracks/<track_id> measure/archive/<track_id>
   ```
   The track folder must no longer exist under `measure/tracks/`.

4. **Update `measure/tracks.md`**
...
5. **Commit the Archive Action**
   ...
   ```bash
   git add measure/archive/<track_id> measure/tracks.md
