 ...
 *
 * Run with either provider path set:
 *   AI_GATEWAY_BASE_URL=http://localhost:3030/api/v1 AI_GATEWAY_MODEL=openai/gpt-4o-mini bun smoke-releaseparser.ts
...
for (const { input, expectMatchType, expectSeason, expectYear, expectType } of parseCases) {
    ...
    if (expectSeason !== undefined) {
        if (result.seasonNumber === expectSeason)
            ok('seasonNumber', result.seasonNumber);
        else
            fail('seasonNumber', result.seasonNumber, String(expectSeason));
    }
    if (expectYear !== undefined) {
        if (result.year === expectYear)
            ok('year', result.year);
        else
            fail('year', result.year, String(expectYear));
    }
    if (expectType !== undefined) {
        if (result.type === expectType)
            ok('type', result.type);
        else
            fail('type', result.type, expectType);
    }
...
const tvContext = { seriesTitle: 'The Big Bang Theory', seasonNumber: 2, preferredResolution: '1080p' };
...
const movieContext = { movieTitle: 'Oppenheimer', preferredResolution: '2160p' };
...
console.log('\n');
//# sourceMappingURL=smoke-releaseparser.js.map