...
export function registerMovieRoutes(
  ...
  app.get('/api/movies/missing', {
    ...
    const items = movies.map((movie: any) => ({
      ...
      cinemaDate: movie.inCinemas?.toISOString?.() ?? movie.cinemaDate ?? undefined,
      physicalRelease: movie.physicalRelease?.toISOString?.() ?? movie.physicalRelease ?? undefined,
      digitalRelease: movie.digitalRelease?.toISOString?.() ?? movie.digitalRelease ?? undefined,
      qualityProfileId: movie.qualityProfileId,
      qualityProfileName: movie.qualityProfile?.name ?? undefined,
