...
describe('WantedSearchService — autoSearchEpisode candidate episode-number validation', () => {
  ...
  it('accepts a multi-episode release when searching for the SECOND episode (S01E01E02 for E02)', async () => {
    // Episode 20 = S01E02 of Breaking Bad
    const episodeFindUnique = vi.fn().mockResolvedValue({
      id: 20,
      episodeNumber: 2,
      seasonNumber: 1,
    ...
    const { episode: _ep, ...restPrisma } = (makeService() as any);
    const prisma = {
      ...restPrisma,
      episode: { findUnique: episodeFindUnique },
    } as unknown as ConstructorParameters<typeof WantedSearchService>[1];
    ...
    const emit = vi.fn().mockResolvedValue(undefined);
    const activityEventEmitter = { emit } as unknown as ActivityEventEmitter;
    const svc = new WantedSearchService(mediaSearchService, prisma, activityEventEmitter);

    const multiEp = makeCandidate('Breaking.Bad.S01E01E02.1080p.BluRay.mkv', 90);
    searchAllIndexers.mockResolvedValue({ releases: [multiEp] });

    const result = await svc.autoSearchEpisode(20);

    // Before fix: regexFallback returns episodeNumbers:[1], so E02 search rejects this release
    // After fix: regexFallback returns episodeNumbers:[1,2], so E02 search accepts it
    expect(result.success).toBe(true);
    expect(grabRelease).toHaveBeenCalledTimes(1);
  });

  it('rejects a multi-episode release when the requested episode is NOT in the list (S01E01E02 for E03)', async () => {
    // Episode 30 = S01E03 of Breaking Bad
    const episodeFindUnique = vi.fn().mockResolvedValue({
      id: 30,
      episodeNumber: 3,
      seasonNumber: 1,
    ...
    const result = await svc.autoSearchEpisode(30);
